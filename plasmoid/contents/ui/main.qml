...
PlasmoidItem {
    ...
    // Was read live from plasmaparc's [General]VolumeStep via a C++
    // GlobalConfig singleton; that required a compiled plugin, which
    // doesn't fit KanvasPlayer's plain-QML package. 5 matches plasmaparc's
    // own default, just without live-updating if you change it system-wide.
